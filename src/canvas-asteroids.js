...
function updateShip()
{
	...
	if(keyLeft) ship.angle -= 0.075;
	if(keyRight) ship.angle += 0.075;
...
function updateBullets()
{
	...
	for(i; i > -1; --i)
	{
		...
		if(b.pos.getX() > screenWidth + b.radius) b.pos.setX(-b.radius);
		else if(b.pos.getX() < -b.radius) b.pos.setX(screenWidth + b.radius);
		
		//if(b.pos.getY() > screenHeight) b.blacklisted = true;
		//else if(b.pos.getY() < 0) b.blacklisted = true;
		if(b.pos.getY() > screenHeight + b.radius) b.pos.setY(-b.radius);
		else if(b.pos.getY() < -b.radius) b.pos.setY(screenHeight + b.radius);
	}
}
...
function updateAsteroids()
{
	...
	for(i; i > -1; --i)
	{
		...
		{
			case 'b':
				asteroidValue = asteroidValue + 4;
			break;
	
			case 'm':
				asteroidValue = asteroidValue + 2;
			break;

			case 's':
				asteroidValue = asteroidValue + 1;
			break;
		}
...
function resolveAsteroidType(asteroid)
{
	switch(asteroid.type)
	{
		case 'b':
		offset = 22;
		generateAsteroid(asteroid.pos.getX()+offset, asteroid.pos.getY()-offset, 40, 'm');
		generateAsteroid(asteroid.pos.getX()-offset, asteroid.pos.getY()+offset, 40, 'm');
		...
		case 'm':
		offset = 11;
		generateAsteroid(asteroid.pos.getX()+offset, asteroid.pos.getY()-offset, 20, 's');
		generateAsteroid(asteroid.pos.getX()-offset, asteroid.pos.getY()+offset, 20, 's');
